...
// ✅ FIX: Fallback ke system theme saat class tidak ada (= light di next-themes)
function getDocumentTheme(): Theme {
  if (typeof document === 'undefined') return 'light'
  if (document.documentElement.classList.contains('dark')) return 'dark'
  return 'light' // tidak ada class dark = light
}

// ✅ FIX: Selalu update state saat class berubah, tidak skip saat null
function useResolvedTheme(themeProp?: 'light' | 'dark'): Theme {
  const [detectedTheme, setDetectedTheme] = useState<Theme>(
    () => getDocumentTheme(), // tidak perlu ?? getSystemTheme() lagi
  )
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      setDetectedTheme(getDocumentTheme()) // ← selalu set, tidak ada guard if
    })
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Setiap kali setStyle dipanggil, naikkan counter ini.
  // styleDataHandler hanya akan memproses event dari setStyle yang paling baru,
  // mengabaikan event-event lama yang masih pending dari reload sebelumnya.
  ...
  useEffect(() => {
    ...
    const map = new MapLibreGL.Map({
      ...
      transformRequest: (url, resourceType) => {
        if (resourceType === 'Glyphs' && url.includes('fonts/Open Sans/')) {
          return {
            url: url.replace('fonts/Open Sans/', 'fonts/Open Sans Regular/'),
          }
        }
        return { url }
      },
    ...
    const styleDataHandler = () => {
      // Snapshot epoch saat event ini masuk
      const epoch = styleEpochRef.current
      clearStyleTimeout()
      // Delay singkat agar Maplibre selesai memproses style sebelum
      // children boleh mulai addSource/addLayer kembali.
      styleTimeoutRef.current = setTimeout(() => {
        // Abaikan jika sudah ada setStyle lebih baru yang belum selesai
  ...
  useEffect(() => {
    ...
    // Naikkan epoch — styleDataHandler dari reload sebelumnya akan diabaikan
    styleEpochRef.current += 1
    // Set false dulu — children akan berhenti beroperasi selama reload
    setIsStyleLoaded(false)

    // ⚠️ Jangan pakai { diff: true } — Maplibre tetap menghapus semua
    // custom source & layer saat URL style berbeda total (light ↔ dark),
    // tapi mengembalikan truthy pada getSource() sebentar sebelum benar-benar
    // dihapus sehingga children salah masuk branch setData, bukan addSource.
    // Tanpa diff, Maplibre melakukan full reload yang bersih dan predictable.
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      paint: {
        ...
        // ✅ Pastikan panjang array > 0 sebelum dipasang
        ...(dashArray &&
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
  ...
  useEffect(() => {
    // 🚀 1. Amankan guard clause di awal
    if (!isLoaded || !map || !map?.getLayer?.(layerId)) return

    // 🚀 2. Amankan properti Paint di dalam loop
    ...
    // 🚀 3. Amankan properti Layout di dalam loop
    ...
    // 🚀 4. Amankan pengecekan dan eksekusi hitLayer
...
> = {
  ...
  pointColor?: any
  /** Stroke/Border color for unclustered individual points. Supports hex or Mapbox expressions. */
  pointStrokeColor?: any
  /** Stroke/Border width for unclustered points. */
  pointStrokeWidth?: number | any
  onPointClick?: (
    feature: GeoJSON.Feature<GeoJSON.Point, P>,
    coordinates: [number, number],
  ) => void
  /**
   * Callback ketika kursor masuk ke area titik pelanggan (Hover Enter).
   * Menerima fungsi `cancelHide` — panggil ini dari onMouseEnter elemen popup
   * agar timer penutupan otomatis dibatalkan saat kursor berpindah ke popup.
   *
   * Contoh penggunaan di parent:
   * ```tsx
   * onPointMouseEnter={(feature, coords, cancelHide) => {
   *   setHovered(feature)
   *   popupCancelHideRef.current = cancelHide  // simpan untuk dipakai di popup
   * }}
   * ```
   */
  onPointMouseEnter?: (
  ...
  /**
   * Callback ketika kursor keluar dari area titik pelanggan (Hover Leave).
   * Penutupan tidak langsung terjadi — ada delay 120ms untuk memberi waktu
   * kursor berpindah ke popup. Popup dapat membatalkan timer ini via `cancelHide`.
   */
...
// HOVER_HIDE_DELAY: jeda (ms) sebelum popup ditutup saat kursor meninggalkan titik.
// Nilai ini harus cukup besar agar kursor sempat berpindah dari titik ke popup
// tanpa popup keburu menghilang. 120ms terasa cepat bagi user tapi cukup untuk
// satu frame pointer-event.
...
>({
  ...
  // 1. Tambah source dan layers saat komponen pertama kali dimuat (Mount)
  useEffect(() => {
    if (!isLoaded || !map) return
    ...
    return () => {
      ...
      // 🛡️ BENTENG UNMOUNT: Jangan sentuh map jika internal style MapLibre sudah hancur
  ...
  // Memaksa layer titik pelanggan selalu berada di paling atas stack map
  useEffect(() => {
    if (!isLoaded || !map) return

    const bringToFront = () => {
      // 🛡️ Amankan dari kondisi unmount/pindah rute mendadak
      if (!map || typeof map.getStyle !== 'function' || !map.getStyle()) return

      if (map.getLayer?.(clusterLayerId)) map.moveLayer?.(clusterLayerId)
      if (map.getLayer?.(clusterCountLayerId))
        map.moveLayer?.(clusterCountLayerId)
      if (map.getLayer?.(unclusteredLayerId))
        map.moveLayer?.(unclusteredLayerId)
    }

    bringToFront()
    const timer = setTimeout(bringToFront, 300)
  ...
  // 2. Perbarui data source jika ada perubahan data (DIAMANKAN)
  useEffect(() => {
    if (!isLoaded || !map || typeof data === 'string') return

    // 🛡️ Jangan perbarui data jika MapLibre sudah mulai unmount/ganti rute
  ...
  // 3. Perbarui style layer secara dinamis jika warna/threshold berubah (DIAMANKAN)
  useEffect(() => {
    if (!isLoaded || !map) return

    // 🛡️ Jangan manipulasi style jika MapLibre sedang dihancurkan
    if (typeof map.getStyle !== 'function' || !map.getStyle()) return
    ...
    try {
      ...
      if (map.getLayer?.(unclusteredLayerId)) {
        if (JSON.stringify(prev.pointColor) !== JSON.stringify(pointColor)) {
          map.setPaintProperty?.(unclusteredLayerId, 'circle-color', pointColor)
        }
        if (
          JSON.stringify(prev.pointStrokeColor) !==
          JSON.stringify(pointStrokeColor)
        ) {
          map.setPaintProperty?.(
            unclusteredLayerId,
            'circle-stroke-color',
        ...
        if (prev.pointStrokeWidth !== pointStrokeWidth) {
          map.setPaintProperty?.(
            unclusteredLayerId,
            'circle-stroke-width',
            pointStrokeWidth,
  ...
  useEffect(() => {
    ...
    const handlePointClick = (e: any) => {
      ...
      if (onPointClick) onPointClick(feature, coordinates)
    }
    ...
    const handlePointMouseEnter = (e: any) => {
      ...
      onPointMouseEnter(feature, coordinates, cancelHide)
    }
    ...
    const handlePointMouseLeave = () => {
      ...
      hideTimerRef.current = setTimeout(() => {
        hideTimerRef.current = null
        if (onPointMouseLeave) onPointMouseLeave()
      }, HOVER_HIDE_DELAY)
    }
